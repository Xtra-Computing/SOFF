...
parser.add_argument('-s', '--smc', action='store_true', help="Whether to use SMC")
...
partyImdb = sy.VirtualWorker(hook, id="imdb")
partyAmazon = sy.VirtualWorker(hook, id="amazon")
partyCrypto = sy.VirtualWorker(hook, id="crypto")
...
#                encryptedParam1 = param1.data.cpu().fix_precision().share(partyImdb, partyAmazon, crypto_provider=partyCrypto)
#                encryptedParam2 = dict_params2[name1].data.cpu().fix_precision().share(partyImdb, partyAmazon, crypto_provider=partyCrypto)
#                dict_params2[name1].data.copy_((beta * encryptedParam1 + (1-beta) * encryptedParam2).get().float_precision())
            params1 = list(netImdb.parameters())
            params2 = list(netAmazon.parameters())
            newParams = list()

            for i in range(len(params1)):
                spdzParams = list()
                spdzParams.append(params1[i].copy().cpu().fix_precision().share(partyImdb, partyAmazon, crypto_provider=partyCrypto))
                spdzParams.append(params2[i].copy().cpu().fix_precision().share(partyImdb, partyAmazon, crypto_provider=partyCrypto))

                newParam = (round(100 * beta) * spdzParams[0]  + round(100*(1-beta)) * spdzParams[1]).get().float_precision() / 100
...
#            exit(1)
            ...
            with torch.no_grad():
                for i in range(len(params1)):
                    list(net.parameters())[i].set_(newParams[i].cuda())
    ...
    print('Aggregation time:', round(aggregationEndTime - aggregationStartTime, 2), 's')

    epochEndTime= time.time()
    print('Epoch time:', round(epochEndTime - epochStartTime, 2), 's')
...
endTime = time.time()
print('Total time:', round(endTime - startTime, 2), 's')
