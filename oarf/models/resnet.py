...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10,
                 track_running_stats=False, *_, **__):
    ...
    def _make_layer(
            self, block, planes, num_blocks, stride, track_running_stats):
        strides = [stride] + [1] * (num_blocks - 1)
        layers = []
        for stride in strides:
            layers.append(
                block(self.in_planes, planes, stride, track_running_stats))
...
# DEBUG
def test(track_running_stats=False):
    net = ResNet18(track_running_stats=track_running_stats)
