...
parser.add_argument('--patience', default=2, type=int, help="Patience for early stopping")
parser.add_argument('--patience-reducelr', default=0, type=int, help="Patience for ReduceLROnPlateau")
parser.add_argument('--min_delta', default=1e-4, type=float, help="Min delta for early stopping and ReduceLROnPlateau")
parser.add_argument('--save-model', action='store_true', default=True, help="Save models")
...
tst_combined_dataset = ConcatDataset(tst_party_datasets)
tst_combined_loader = DataLoader(tst_combined_dataset, batch_size=args.val_batch_size, shuffle=False, **kwargs)
tst_loaders.append(tst_combined_loader)
...
def val(loaders):
    ...
    loss_avg = avg(losses, val_lengths)
    metric_avg = avg(metrics, val_lengths)
    print('Val loss_avg {:.4f}, metric_avg {:.4f}'.format(loss_avg, metric_avg))
    
    if args.setting != 'fedavg' and args.setting != 'combined':
        loss_avg = losses[int(args.setting)]
...
def tst(loaders):
    ...
    for i, loader in enumerate(loaders):
        ...
        right_features = None
        labels = None
        for j, (i1, i1f, i2, i2f, lab) in enumerate(loader):
            i1, i1f, i2, i2f = i1.to(device), i1f.to(device), i2.to(device), i2f.to(device)
            o1 = model(i1, 'tst').clone().detach().cpu().numpy()
            o1f = model(i1f, 'tst').clone().detach().cpu().numpy()
            o2 = model(i2, 'tst').clone().detach().cpu().numpy()
            o2f = model(i2f, 'tst').clone().detach().cpu().numpy()

            left = np.concatenate((o1, o1f), axis=1)
            right = np.concatenate((o2, o2f), axis=1)
            ...
            if j == 0:
                ...
                left_features = np.vstack((left_features, left))
                right_features = np.vstack((right_features, right))
        ...
        for train_index, test_index in kf.split(labels):
            veclist1 = left_features[train_index]
            veclist2 = right_features[train_index]
            veclist = np.concatenate((veclist1, veclist2), axis=0)
            mean = np.mean(veclist, axis=0)
            veclist1 -= mean
            veclist2 -= mean
            dist = np.sum(veclist1 * veclist2, axis=1)
            dist = dist / line.norm(veclist1, axis=1) / line.norm(veclist2, axis=1)
            acc = np.zeros_like(thresholds)
            for thresindex, thres in enumerate(thresholds):
                predict_issame = np.less(dist, thres)
                actual_issame = np.less(labels[train_index], 0.5)
                acc[thresindex] = np.sum(np.logical_and(predict_issame, actual_issame)) + np.sum(np.logical_and(np.logical_not(predict_issame), np.logical_not(actual_issame)))
            thres_count = 0
            maxacc = np.max(acc)
            thres_sum = 0
            for index in range(len(thresholds)):
                if acc[index] == maxacc:
                    thres_count += 1
                    thres_sum += thresholds[index]
            best_thres = thres_sum / thres_count

            veclist1 = left_features[test_index]
            veclist2 = right_features[test_index]
            veclist1 -= mean
            veclist2 -= mean
            dist = np.sum(veclist1 * veclist2, axis=1)
            dist = dist / line.norm(veclist1, axis=1) / line.norm(veclist2, axis=1)
            predict_issame = np.less(dist, best_thres)
            actual_issame = np.less(labels[test_index], 0.5)
            res = np.sum(np.logical_and(predict_issame, actual_issame)) + np.sum(np.logical_and(np.logical_not(predict_issame), np.logical_not(actual_issame)))
            accres = float(res) / float(len(test_index))
            accuracy[fold_index] = accres
            fold_index += 1

        accres = np.mean(accuracy)
        metrics.append(accres)
        
    metric_print = 'Tst metric: '
    for i in range(num_parties):
        metric_print += '{:4f} '.format(metrics[i])
    print(metric_print)
    print('Tst metric_combine {:.4f}'.format(metrics[-1]))
...
for epoch in range(args.starting_epoch, args.epochs):
    ...
    t3 = int(time.time())
    print('Epoch trn time {:d}s, val & tst time {:d}s'.format(t2-t1, t3-t2))
    cnt += 1
