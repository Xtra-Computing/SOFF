...
def unpack_tensor(data: bytearray, offset: Offset) -> torch.Tensor:
    ...
    nparray = np.frombuffer(
        unpack_raw_data(data, offset), dtype).reshape(shape)
...
def pack_raw_data_list(raw_data_list: List[bytearray]) -> bytearray:
    data = bytearray()
    data.extend(pack_int(len(raw_data_list)))
    for raw_data in raw_data_list:
        data.extend(pack_raw_data(raw_data))
    return data


def unpack_raw_data_list(data: bytearray, offset: Offset) -> List[bytearray]:
    raw_data_list = []
    count = unpack_int(data, offset)
    for i in range(count):
        raw_data_list.append(unpack_raw_data(data, offset))
...
def unpack_fixed_width_data(data: bytearray, offset: Offset,
    ...
    """
    result = copy(data[offset:offset + width])
    offset += width
    return result
...
def unpack_int(buf: bytearray, offset: Offset) -> int:
    ...
    if dtype == np.int8:
        ...
        raise ValueError(f"Unknown value type: {dtype}")
    return val
