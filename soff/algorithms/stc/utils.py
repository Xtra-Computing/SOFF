...
def create_stc_compressor(
        net: nn.Module, ratio: float, cfg: Munch) -> TopKPerLayer:

    if cfg.model.name not in stc_non_compress_params:
    ...
    for i in range(stc_non_compress_params[cfg.model.name][0]):
        grad_topk_ratios[i] = 1.
    for i in range(stc_non_compress_params[cfg.model.name][1]):
...
def stc_ae_param_idxs(net: nn.Module) -> List[int]:
    """Filter network parameter indices for stc's autoencoder training"""
...
def golomb_idx_size(cfg: Munch, net: nn.Module, sparse_rate: float):
    ...
    start = stc_non_compress_params[cfg.model.name][0]
    end = stc_non_compress_params[cfg.model.name][0]
