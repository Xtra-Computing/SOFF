...
class PlainCoder(EntropyCoder):
    ...
    def __init__(self, cfg: Munch):
        self.total_codewords = cfg.ugc.quantizer.quantization_level
        if cfg.ugc.quantizer.quantization_level == 2:
            # sign bits are overlapped with level bits
            self.ratio_ = 0
        elif cfg.ugc.quantizer.quantization_level == 0:
...
class IdealCoder(EntropyCoder):
    ...
    def __init__(self, cfg: Munch):
        ...
        self.total_codewords = int(2 ** np.ceil(
            np.log2(cfg.ugc.quantizer.quantization_level)))
...
class ArithmeticCoder(EntropyCoder):
    ...
    def __init__(self, cfg: Munch):
        # self.symbs = cfg.ugc.quantizer.quantization_level
...
class UGCEncoderConfParser(BaseConfParser):
