...
class PlainCoder(EntropyCoder):
    ...
    def __init__(self, cfg: Munch):
        self.total_codewords = cfg.qsgd.quantizer.quantization_level
        if cfg.qsgd.quantizer.quantization_level == 2:
            # sign bits are overlapped with level bits
            self.ratio_ = 0
        elif cfg.qsgd.quantizer.quantization_level == 0:
    ...
    def encode(self, result):
        ...
        return pack_tensor(result.norms.cpu()) + pack_tensor(tensor.cpu())
...
class IdealCoder(EntropyCoder):
    ...
    def __init__(self, cfg: Munch):
        ...
        self.total_codewords = int(2 ** np.ceil(
            np.log2(cfg.qsgd.quantizer.quantization_level)))
    ...
    def encode(self, result):
        ...
        """
        return pack_tensor(result.norms.cpu()) + pack_tensor(result.quantized_tensor.cpu())
...
class ArithmeticCoder(EntropyCoder):
    ...
    def __init__(self, cfg: Munch):
        # self.symbs = cfg.qsgd.quantizer.quantization_level
