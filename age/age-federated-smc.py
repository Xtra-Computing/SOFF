...
parser.add_argument('-smc',
                    '--smc',
                    action="store_true",
                    help="Whether to use SMC")
...
## Initialize SMC environment ##################################################
import syft as sy
hook = sy.TorchHook(torch)
parties = [sy.VirtualWorker(hook, id=dataset) for dataset in args.datasets]
...
for e in range(starting_epoch, starting_epoch + args.epochs):
    ...
    if not (e + 1) % args.average_every == 0:
        continue

    aggregationStartTime = time.time()
    if args.smc:
        safe_print("Thread main: Strating aggregation with SMC")
        total_num_data = sum(
            [len(dataset['train']) for dataset in datasets_splitted])
        ratios = [
            len(dataset['train']) / total_num_data
            for dataset in datasets_splitted
        ]

        smc_precision = 2
        multiplyer = 10**(smc_precision + 2)

        params_list = [list(net.parameters()) for net in nets]
        new_params = list()
        for i in range(len(params_list[0])):
            spdz_params = list()
            for party_idx, params in enumerate(params_list):
                spdz_params.append(params[i].copy().cpu().fix_precision().share(
                    *parties,
                    crypto_provider=partyCrypto))

            for party_idx, param in enumerate(spdz_params):
                if party_idx == 0:
                    new_param = round(multiplyer * ratios[party_idx]) * param
                else:
                    new_param = new_param + round(multiplyer * ratios[party_idx]) * param
            new_param = new_param.get().float_precision() / multiplyer
            new_params.append(new_param)

        with torch.no_grad():
            for i, net in enumerate(nets):
                for j in range(len(new_params)):
                    list(net.parameters())[i].set_(new_params[i].cuda(
