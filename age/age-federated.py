...
parser = argparse.ArgumentParser(
    description='train with fedavg and dp', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument('-E', '--epochs', default=50,
                    type=int, help="Number of epochs")
parser.add_argument('-d', '--data-dir', default="preprocessed_data",
                    help="The directory storing preprocessed data")
parser.add_argument('-b', '--batch-size', default=50,
                    type=int, help="How many record per batch")
parser.add_argument('-a', '--average-every', default=1,
                    type=int, help="How many epochs per averaging")
parser.add_argument('-s', '--save-dir', default="saves",
                    type=str, help="Model save dir")
parser.add_argument('-l', '--load-model', type=str,
                    help="Load previous save point, must also specify starting epoch")
parser.add_argument('-ee', '--ending-epoch', type=int,
                    help="Last ending epoch number. Used for continuing from last save")
parser.add_argument('-ds', '--datasets', default=[
                    'allagefaces', 'appa'], type=str, nargs='+', help='|'.join(available_datasets))
parser.add_argument('-sc', '--scatter', action="store_true",
                    help="Scatter to multple GPUs")
parser.add_argument('-ls', '--lr-scheduler', default='step',
                    type=str, help="Which lr schedular to use: <step|plateau>")
...
for net in nets:
    optimizers.append(
        torch.optim.SGD([
            ...
            {'params': net.classifier[6].parameters(), 'lr': 0.001},
        ],
        lr=learningRate,
        momentum=0.9,
        weight_decay=0.0005))
...
for optimizer in optimizers:
    if args.lr_scheduler == 'step':
        ...
        lr_schedulers.append(torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=10, verbose=True))
    else:
        raise Exception("unknown lr_scheduler")
...
clients_finished = threading.Event()
...
def training_on_one_gpu(gpu_idx):
    for e in range(starting_epoch, starting_epoch + args.epochs):
        ...
        # end when learning rate is reduced 4 times
        if args.lr_scheduler == 'plateau' and optimizers[0].param_groups[0]['lr'] < learningRate * (0.1 ** 4) * 1.01:
            ending_events[gpu_idx].set()
            clients_finished.set()
            return
        ...
        for data_idx in filter(lambda x: x % len(devices) == gpu_idx,
            ...
            elif args.lr_scheduler == 'plateau' and (e + 1) % args.average_every == 0:
                lr_schedulers[data_idx].step(global_loss)
...
for e in range(starting_epoch, starting_epoch + args.epochs):
    ...
    safe_print("learning rate: {}".format(optimizers[0].param_groups[0]['lr']))
    ...
    if clients_finished.is_set():
        break

    if not ((e + 1) % args.average_every == 0 or (e + 1) == starting_epoch + args.epochs):
        continue
    ...
    global_loss=np.average(losses_eval).item()
