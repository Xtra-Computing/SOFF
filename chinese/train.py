...
parser.add_argument('--patience-reducelr', default=10, type=int, help="Patience for ReduceLROnPlateau")
parser.add_argument('--min_delta', default=1e-4, type=float, help="Min delta for early stopping and ReduceLROnPlateau")
parser.add_argument('--save-freq', default=5, type=int, help="Save model per how many epochs")

parser.add_argument('--no-gpu', action='store_true', default=False, help="Not use gpu")
...
trn_party_datasets, val_party_datasets, tst_party_datasets = get_datasets(args)  # each party's loader and the combined loader
...
if args.dp:
    party_samplers = [RandomSampler(trnset, replacement=True) for trnset in trn_party_datasets]
    ...
    trn_combined_loader = DataLoader(trn_combined_dataset, sampler=combined_sampler, batch_size=args.batch_size, shuffle=False, **kwargs)
else:
...
avged = True  # set to True when fedavg is done; set to False when separately training on two parties
def fedavg():
...
# save model to
model_dir = '{}/models/setting-{}-epochs-{}-localepochs-{}'.format(args.task_dir, args.setting, args.epochs, args.local_epochs)
if args.spdz:
    model_dir += '-spdz'
if args.dp:
...
for epoch in range(args.starting_epoch, args.epochs):
    ...
    train(epoch)
    t2 = int(time.time())
    val(epoch, val_loaders)
    val(epoch, tst_loaders)
    if args.save_freq > 0 and epoch % args.save_freq == args.save_freq - 1:
